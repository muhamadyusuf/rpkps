...
export async function tambahKunci(data: FormData): Promise<Hasil> {
  ...
  try {
    ...
    if (hasil.ok) revalidatePath("/pengaturan/ai");
    return { ok: hasil.ok, pesan: hasil.pesan };
  } catch (galat) {
    return { ok: false, pesan: pesanGalat(galat, "Gagal menyimpan kunci.") };
  }
}
...
export async function ujiKunciBaru(
  ...
  if (apiKey.trim().length < 16) {
    return { ok: false, pesan: "Kunci API terlalu pendek untuk sah." };
  }
  try {
